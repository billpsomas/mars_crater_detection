...
void runDPU(vart::Runner* runner, int8_t *imageInputs, int8_t *FCResult, int8_t *FCResult1, int8_t *FCResult2)
{
  ...
  int8_t *loc_FCResult1    = FCResult1;
  int8_t *loc_FCResult2    = FCResult2;
  ...
  for (unsigned int n = 0; n < num_images_x_thread; n += batchSize) //this works correctly for either batchSize= 1 or 3
  {
    ...
    loc_FCResult1   = FCResult1   +n * outSize1;
    loc_FCResult2    = FCResult2   +n * outSize2;
    ...
    batchTensors.push_back(std::shared_ptr<xir::Tensor>(xir::Tensor::create(outputTensors[1]->get_name(),
          out_dims1,xir::DataType{xir::DataType::XINT, 8u})));

    outputs.push_back(std::make_unique<CpuFlatTensorBuffer>( loc_FCResult1, batchTensors.back().get()));

    batchTensors.push_back(std::shared_ptr<xir::Tensor>(xir::Tensor::create(outputTensors[2]->get_name(),
          out_dims2,xir::DataType{xir::DataType::XINT, 8u})));

    outputs.push_back(std::make_unique<CpuFlatTensorBuffer>( loc_FCResult2, batchTensors.back().get()));
...
int main(int argc, char* argv[])
{
  ...
  int outSize1   = shapes.outTensorList[1].size;
  int outHeight1 = shapes.outTensorList[1].height;
  int outWidth1  = shapes.outTensorList[1].width;

  int outSize2  = shapes.outTensorList[2].size;
  int outHeight2 = shapes.outTensorList[2].height;
  int outWidth2  = shapes.outTensorList[2].width;
  ...
  auto output_scale1 = get_output_scale(outputTensors[1]);
  auto output_scale2 = get_output_scale(outputTensors[2]);

  // debug messages
  cout << "outSize   " << outSize << endl;
  
  cout << "outSize1   " << outSize1 << endl;
  cout << "outSize2   " << outSize2 << endl;  
  
  cout << "inSize    " << inSize << endl;
  cout << "outW      " << outWidth << endl;
  cout << "outH      " << outHeight << endl;
  
  cout << "outW 1     " << outWidth1 << endl;
  cout << "outH 1     " << outHeight1 << endl;

  cout << "outW 2     " << outWidth2 << endl;
  cout << "outH 2     " << outHeight2 << endl;  
  ...
  cout << "out scale1 " << output_scale1 << endl;
  cout << "out scale2 " << output_scale2 << endl;  
  ...
  cout << "\n allocated " << num_of_images*outSize1 << " bytes for output buffer " << endl;
  cout << "\n allocated " << num_of_images*outSize2 << " bytes for output buffer " << endl;  
  ...
  int8_t  *FCResult01    = FCResult_1+   outSize1*(num_threads==1 ? 0*num_images_x_thread : 0);
  int8_t  *FCResult11    = FCResult_1+   outSize1*(num_threads==2 ? 1*num_images_x_thread : 0);
  int8_t  *FCResult21    = FCResult_1+   outSize1*(num_threads==3 ? 2*num_images_x_thread : 0);
  int8_t  *FCResult31    = FCResult_1+   outSize1*(num_threads==4 ? 3*num_images_x_thread : 0);
  int8_t  *FCResult41    = FCResult_1+   outSize1*(num_threads==5 ? 4*num_images_x_thread : 0);
  int8_t  *FCResult51    = FCResult_1+   outSize1*(num_threads==6 ? 5*num_images_x_thread : 0);

  int8_t  *FCResult02    = FCResult_2+   outSize2*(num_threads==1 ? 0*num_images_x_thread : 0);
  int8_t  *FCResult12    = FCResult_2+   outSize2*(num_threads==2 ? 1*num_images_x_thread : 0);
  int8_t  *FCResult22    = FCResult_2+   outSize2*(num_threads==3 ? 2*num_images_x_thread : 0);
  int8_t  *FCResult32    = FCResult_2+   outSize2*(num_threads==4 ? 3*num_images_x_thread : 0);
  int8_t  *FCResult42    = FCResult_2+   outSize2*(num_threads==5 ? 4*num_images_x_thread : 0);
  int8_t  *FCResult52    = FCResult_2+   outSize2*(num_threads==6 ? 5*num_images_x_thread : 0);
  ...
  auto dpu_t1 = std::chrono::high_resolution_clock::now();
    ...
    {
        if (i == 0) workers[i] = thread(runDPU, runner.get(),   ref(imagesInput0), ref(FCResult0), ref(FCResult01), ref(FCResult02) );
        if (i == 1) workers[i] = thread(runDPU, runner1.get(),  ref(imagesInput1), ref(FCResult1), ref(FCResult11), ref(FCResult12) );
        if (i == 2) workers[i] = thread(runDPU, runner2.get(),  ref(imagesInput2), ref(FCResult2), ref(FCResult21), ref(FCResult22) );
        if (i == 3) workers[i] = thread(runDPU, runner3.get(),  ref(imagesInput3), ref(FCResult3), ref(FCResult31), ref(FCResult32) );
        if (i == 4) workers[i] = thread(runDPU, runner4.get(),  ref(imagesInput4), ref(FCResult4), ref(FCResult41), ref(FCResult42) );
        if (i == 5) workers[i] = thread(runDPU, runner5.get(),  ref(imagesInput5), ref(FCResult5), ref(FCResult51), ref(FCResult52) );
    }
