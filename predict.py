...
parser.add_argument('--model_name', type=str, default="/mnt/data/bill/code/yolov8/runs/detect/yolov8n-mars-imgsz-256-batch-512/weights/best.pt", help='path to the model weights')
...
if args.save_txts:
    txts_dir = os.path.join(args.save_dir, 'txts')
...
for res in results:
    ...
    if args.save_imgs:
        ...
        im.save(os.path.join(imgs_dir, im_id + '.png'))  # save image

    if args.save_txts:
        res.save_txt(os.path.join(txts_dir, res.path.split('/')[-1].replace('.png', '.txt'))) # save txt 
